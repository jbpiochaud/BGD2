...
ax.scatter(X, y, color='black', s=300, c=purple,
...
my_saving_display(fig1, dirname, "standardNorm_multiple", imageformat)
...
my_saving_display(fig2, dirname, "standardNorm_multiple_bis", imageformat)
...
def covmat_to_scalar(sigma):
    ...
    sigmaxy = sigma[1, 0] / (sigmax * sigmay)
...
def angle_scalar_to_covmat(theta, sig1, sig2):
    ...
    sigma = np.dot(np.dot(rotation, np.diag([sig1, sig2])), rotation.T)
    return sigma


xx = np.linspace(-6, 6, step)
yy = xx
...
ax.set_zlim(0, 0.2)
plt.show()
my_saving_display(fig1, dirname, "aniso_gaussian", imageformat)
...
ax.set_zlim(0, 0.2)
plt.show()
my_saving_display(fig1, dirname, "rotated_gaussian", imageformat)

#######################
# Plot neg rotated case
theta = pi / 2
sigma = angle_scalar_to_covmat(theta, sig1, sig2)
sigmax, sigmay, sigmaxy = covmat_to_scalar(sigma)
Z2_ter = plt.mlab.bivariate_normal(Xg, Yg, sigmax=sigmax,
...
my_saving_display(fig1, dirname, "rotated_neg_gaussian", imageformat)
